...
class MyLlamaCppWithTools(MyLlamaCpp):
    ...
    def _messages_to_prompt_with_tools(self, messages):
        ...
        if self._bound_tools:
            system_msg = f"""You are a helpful, smart, and efficient assistant with access to tools. You are also an expert to analysis the corrugated paper system.
            ...
            When you decide to answer, you should try to format it into structured data.
            ...
            **Corrugated Paper System Rules:**
            1. LS0, MS1, LS1, MS2, LS2 are important parts in the system, and paper material is set to these parts. For example, material P.-.-.8.J means the material in LS0 is P, and none in MS1, None in LS1, 8 in MS2, J in LS2.
            2. Material can be represented like (P,2600,B) or (P.-.-.8.J), (P, 2600, B) means (material, width, fluteType) for a single part, and (P.-.-.8.J) follows rule number 1.
            3. In system, we have 2 types of events, set function event and material change event.
            4. When the user request the answer about the corrugated paper system log, try to call the tool you have, and make the answer based on the tool result.
            """
...
def improved_call_tools(state, llm_with_tools, tools):
    ...
    state['tool_calls_and_results'].append(tool_calls[0])
    
    tool_results = llm_with_tools.execute_tool_calls(tool_calls, tools)

    state['tool_calls_and_results'].append(tool_results[0].content)
