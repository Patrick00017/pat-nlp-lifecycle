...
from networkx_construct import load_graph, query_node_relationships, query_all_nodes, extract_upward_subgraph
...
# model = ChatLlamaCpp(
#     temperature=0.5,
#     model_path="D:/code/btsagent/models/Qwen3.5-4B-Q8_0.gguf",
#     n_ctx=10000,
#     n_gpu_layers=8,
...
def init_all():
    ...
    # brain storm agent
    graph_fetch_agent = create_agent(model, tools=[], middleware=[prompt_fetch_useful_graph])
    graph_path_summarize_agent = create_agent(model, tools=[], system_prompt="""
        你是一个系统使用培训助手。
        现已将操作相关的部分以图的方式呈现，使用通俗易懂的语句，基于图结构进行操作流程的梳理，不要说明图以外的任何信息，以免对用户产生误导。
...
def response(question):
    ...
    if intent != "未知":
        ...
        subgraph_content = extract_upward_subgraph(loaded_graph, parsed_graph_nodes_output["nodes"], relation_type="属于")
