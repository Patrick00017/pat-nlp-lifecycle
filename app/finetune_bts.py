...
def tokenize_function(example):
    ...
    example["labels"] = tokenizer(
        example["answer"], padding="max_length", truncation=True, return_tensors="pt"
    ).input_ids

    return example


dataset = load_and_inspect("qa.json")
...
print(dash_line)
print(
    f"BASELINE Answer:\n生管控制主界面包括：刀头中控用户、坑机用户（SF1、SF2、SF3）、糊机用户、双面机用户、同步屏用户（刀头中控人员）和办公室人员（排单人员、跟单人员、成品仓物流人员）。"
)
print(dash_line)
