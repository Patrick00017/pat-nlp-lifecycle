...
class MyLlamaCppWithTools(MyLlamaCpp):
    ...
    def _messages_to_prompt_with_tools(self, messages):
        ...
        if self._bound_tools:
            system_msg = f"""
            You are a helpful, smart, and efficient assistant with access to tools.
            ...
            You need to check tool usage rules.
    ...
    def execute_tool_calls(
        self, tool_calls: List[Dict], available_tools: List
    ) -> List[ToolMessage]:
        """
        ...
        for i, call in enumerate(tool_calls):
            ...
            if tool_function:
                try:
                    ...
                    results.append(
                        ToolMessage(
                            content=str(result), tool_call_id=f"{tool_name}_{i}"
                        )
                    )
                    ...
                    results.append(
                        ToolMessage(content=error_msg, tool_call_id=f"{tool_name}_{i}")
                    )
            else:
                error_msg = f"Unknown tool: {tool_name}"
                logger.warning(error_msg)
                results.append(
                    ToolMessage(content=error_msg, tool_call_id=f"unknown_{i}")
                )
...
def improved_should_continue(state, llm_with_tools, max_iterations: int = 20) -> str:
    ...
    if len(messages) > max_iterations:
        logger.warning(
            f"Maximum iterations ({max_iterations}) reached. Ending conversation."
        )
        return "end"
    ...
    tool_calls = llm_with_tools.parse_tool_calls(content)
    result = "check" if tool_calls else "end"
