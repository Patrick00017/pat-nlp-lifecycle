...
class BPETokenizerSimple:
    ...
    def load_vocab_and_merges_from_openai(self, vocab_path, bpe_merges_path):
        ...
        if (
            "<|endoftext|>" not in self.inverse_vocab
            or self.inverse_vocab["<|endoftext|>"] != 50256
        ):
    ...
    def encode(self, text, allowed_special=None):
        ...
        if allowed_special is None:
            ...
            disallowed = [
                tok
                for tok in specials_in_vocab
                if tok in text and tok not in allowed_special
            ]
            if disallowed:
                raise ValueError(
                    f"Disallowed special tokens encountered in text: {disallowed}"
